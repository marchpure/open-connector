...
export function registerOAuthCompatRoutes(app: Hono, options: OAuthCompatOptions): void {
  ...
  app.get("/oauth/authorize", (context) => {
    ...
    const signedState = signState(
      { redirectUri, state: query.state ?? "", clientId: query.client_id },
      options.stateSecret,
    );
  ...
  app.post("/oauth/token", async (context) => {
    ...
    if (!code || !verifier) {
      return writeJson(context, { error: "invalid_request", error_description: "code and code_verifier are required." }, 400);
    }
...
function isAllowedWorkBuddyRedirect(value: string): boolean {
  try {
    const url = new URL(value);
    return url.protocol === "workbuddy:" || (url.protocol === "http:" && ["127.0.0.1", "localhost"].includes(url.hostname));
  } catch {
    return false;
  }
}
