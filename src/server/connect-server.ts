...
export class ConnectServer {
  ...
  createApp(): Hono {
    ...
    if (runtimeEnabled) {
      ...
      app.get("/mcp/tools", async (context) =>
        context.json({
          tools: await listAuthorizedMcpToolSummaries(this.mcpAuthorizerFor(context), {
    ...
    const handler = createMcpHandler(
      () =>
        createMcpServer({
          ...
          authorizer: this.mcpAuthorizerFor(context),
  ...
  private mcpAuthorizerFor(context: Context): McpAuthorizer | undefined {
    return readRuntimeSubject(context) ? undefined : this.mcpAuthorizer;
  }
