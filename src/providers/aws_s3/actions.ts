  ...
  defineProviderAction(service, {
    ...
    inputSchema: s.object(
      "The input payload for this action.",
      {
        ...
        ifMatch: s.string("Only read the object when its ETag matches this value."),
      },
      { optional: ["bucket", "region", "endpoint", "versionId", "ifMatch"] },
    ),
    outputSchema: s.object("The output payload for this action.", {
  ...
  defineProviderAction(service, {
    ...
    inputSchema: s.object(
      "The input payload for downloading one S3 object.",
      {
        ...
        endpoint: endpointField,
        versionId: s.string("The optional object version ID."),
        ifMatch: s.string("Only download the object when its ETag matches this value."),
